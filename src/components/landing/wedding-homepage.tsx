...
const mobileHeroSlides = ["/wedding/valdeze-patrick-hero.jpeg", "/wedding/valdeze-patrick-editorial.jpeg"];
...
function WeddingPageLoader({ leaving }: { leaving: boolean }) {
  return <div aria-live="polite" aria-label="Chargement de la célébration" className={`wedding-loader-scene fixed inset-0 z-[100] grid place-items-center overflow-hidden bg-[#fff9ed] px-6 transition-all duration-1000 ease-[cubic-bezier(.22,1,.36,1)] ${leaving ? "pointer-events-none scale-[1.035] opacity-0" : "opacity-100"}`}>
    <div aria-hidden className="wedding-loader-orb-a absolute -left-28 -top-28 size-96 rounded-full border border-[#c9964e]/30" />
    <div aria-hidden className="wedding-loader-orb-b absolute -bottom-40 -right-24 size-[34rem] rounded-full border border-[#6e1720]/15" />
    <div aria-hidden className="wedding-loader-glow absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(207,152,67,.12),transparent_44%)]" />
    <span aria-hidden className="wedding-loader-spark absolute left-[22%] top-[29%] size-1.5 rounded-full bg-[#b4772a]" />
    <span aria-hidden className="wedding-loader-spark wedding-loader-spark-delayed absolute bottom-[28%] right-[21%] size-1 rounded-full bg-[#6e1720]" />
    <div className="relative flex flex-col items-center text-center">
      <div className="wedding-loader-mark relative grid size-28 place-items-center">
        <span aria-hidden className="absolute inset-0 rounded-full border border-[#b4772a]/55 animate-[wedding-loader-orbit_3s_linear_infinite]" />
        <span aria-hidden className="absolute inset-2 rounded-full border border-dashed border-[#6e1720]/35 animate-[wedding-loader-orbit_5s_linear_infinite_reverse]" />
        <span className="text-4xl text-[#a56722]">V<span className="text-[#6e1720]">&</span>P</span>
      </div>
      <div className="wedding-loader-ornament mt-7 flex items-center gap-3 text-[#b4772a]"><span className="h-px w-10 bg-current/55" /><Heart className="size-3.5 fill-current" /><span className="h-px w-10 bg-current/55" /></div>
      <p className="wedding-loader-name mt-5 text-xs font-semibold uppercase tracking-[.32em] text-[#a56722]">Valdeze & Patrick</p>
      <p className="wedding-loader-title mt-3 text-xl italic text-[#6e1720]">Préparons la célébration</p>
      <p className="wedding-loader-date mt-5 text-[10px] font-semibold uppercase tracking-[.25em] text-[#9b6e35]">30 · 31 octobre 2026</p>
    </div>
  </div>;
}
...
export function WeddingHomepage() {
  ...
  return <><WeddingPageLoader leaving={loaderLeaving} /><main className="min-h-screen overflow-hidden bg-[#fffaf0] font-serif text-[#53151d]">
    <section className="relative isolate min-h-[780px] overflow-hidden bg-[#fff9ed] lg:min-h-[760px]">
      {heroSlides.map((slide, index) => <Image key={slide} src={slide} alt="Valdeze et Patrick" fill priority={index === 0} sizes="100vw" className={`pointer-events-none z-0 hidden object-cover object-[62%_center] transition-opacity duration-[1400ms] md:block lg:object-center ${active === index ? "hero-backdrop-in opacity-100" : "opacity-0"}`} />)}
      {mobileHeroSlides.map((slide, index) => <Image key={slide} src={slide} alt="Valdeze et Patrick" fill priority={index === 0} sizes="100vw" className={`pointer-events-none z-0 object-contain object-bottom transition-opacity duration-[1400ms] md:hidden ${active === index ? "hero-backdrop-in opacity-100" : "opacity-0"}`} />)}
      <div aria-hidden className="pointer-events-none absolute inset-0 z-[1] bg-[linear-gradient(90deg,rgba(255,249,237,.88)_0%,rgba(255,249,237,.70)_68%,rgba(255,249,237,.12)_100%)] md:hidden" />
    ...
    <section id="message" className="relative isolate min-h-[620px] overflow-hidden bg-[#fff9ed] px-5 py-16 sm:px-8"><Image src="/wedding/footer-composed-v1.png" alt="Valdeze et Patrick" fill sizes="100vw" className="-z-10 hidden object-cover object-[50%_top] md:block" /><Image src="/wedding/valdeze-patrick-editorial.jpeg" alt="Valdeze et Patrick" fill sizes="100vw" className="-z-10 object-contain object-top md:hidden" /><div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-[68%] bg-[linear-gradient(to_top,rgba(255,249,237,.98)_33%,rgba(255,249,237,.84)_69%,rgba(255,249,237,0)_100%)] md:hidden" /><div className="relative z-10 mx-auto flex min-h-[490px] max-w-6xl items-end justify-end md:min-h-[460px] md:items-center"><div className="max-w-xl pb-2 text-center md:ml-auto md:w-[54%] md:pb-0 md:text-left"><div className="flex items-center justify-center gap-3 text-[#b4772a] md:justify-start"><span className="h-px w-10 bg-current/45" /><Heart className="size-5" /><span className="h-px w-10 bg-current/45" /></div><p className="mt-5 text-xs font-semibold uppercase tracking-[.28em] text-[#a56722]">Avec tout notre amour</p><h2 className="mt-5 text-4xl leading-[1.05] text-[#6e1720] sm:text-6xl">Votre présence est<br /><span className="italic text-[#b4772a]">notre plus beau cadeau.</span></h2><div className="my-7 h-px w-24 bg-[#b98a46]/70 md:mx-0 mx-auto" /><p className="mx-auto max-w-md text-lg leading-relaxed text-[#67493c] md:mx-0">Nous avons hâte de vivre ces moments inoubliables avec vous, entourés de nos familles et de nos amis.</p><p className="mt-8 text-3xl italic text-[#8e461d]">Valdeze & Patrick</p></div></div></section>
